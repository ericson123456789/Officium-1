...
contract OfficiumToken is StandardToken {

    string public constant name = "Officium";
    ...
    function OfficiumToken(address tokenOwner, uint _endDate) {
...
contract OfficiumCrowdsale is Ownable {
    using SafeMath for uint256;
...
Week 7-8: 1 ETH = 14 000 OFC (0% Bonus)
    **/
    ...
    uint public constant endPreICO = 0;

    uint public constant startICO = 1530457200;
    uint public constant endDate = 1535209200;

    uint constant bonus40 = startICO + 2 weeks;
    uint constant bonus20 = startICO + 4 weeks;
    uint constant bonus10 = startICO + 6 weeks;
    uint constant bonus0 = startICO + 8 weeks;
    ...
    uint public constant IcoSoftCap = 70000000 ether;
    ...
    function OfficiumCrowdsale () {
        tokenAddress = new OfficiumToken(tokenOwner, endDate);
    }
    ...
    function buyTokens() internal {
        ...
        if(isICO()){
            ...
            } else if (bonus40 > now &&  now <= bonus20) {
                tokens += tokens * 20 / 100;
            } else if (bonus20 > now &&  now <= bonus10) {
                tokens += tokens * 10 / 100;
            } else if (bonus10 > now &&  now <= bonus0) {
                tokens += tokens * 0 / 100;
            }    

            uint256 balance = IcoSoftCap.sub(stat.currentFundraiser);
            if (balance < tokens) {
    ...
    function sendTokens(uint _amount, uint _ethers) internal {
        OfficiumToken tokenHolder = OfficiumToken(tokenAddress);
